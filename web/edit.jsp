...
<html>
    ...
    <body>
        ...
        <form action="MainController">
            <c:set var="ProObj" value="${sessionScope.ProObj}" />
            Product ID: <input type="text" name="id" value="${ProObj.idProduct}" readonly="true"/>
            <c:if test="${not empty errorObj.id}" >
                <font color ="red" >${errorObj.id} </font>
            </c:if>
            <br/>
            Product Name: <input type="text" name="name" value="${ProObj.nameProduct}" />
            <c:if test="${not empty errorObj.name}" >
                <font color ="red" >${errorObj.name} </font>
            </c:if>
            <br/>
            Description: <textarea name="des" rows="4" cols="50" >${ProObj.description}</textarea>
            <c:if test="${not empty errorObj.des}" >
                <font color ="red" >${errorObj.des} </font>
            </c:if>
            <br/>
            Price: <input type="text" name="price" value="${ProObj.price}" />
            <c:if test="${not empty errorObj.price}" >
                <font color ="red" >${errorObj.price} </font>
            </c:if>
            <br/>
            Image: <select name="proImage">
            ...
            <br/>
            <input type="hidden" name="proImage" value="${ProObj.image}" />
            <button type="submit" class="btn btn-outline-secondary" value="EDITPRO" name="action">EDIT</button>
        </form>
    </body>
</html>
